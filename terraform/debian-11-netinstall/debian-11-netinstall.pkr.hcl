...
source "proxmox-iso" "debian-11-netinstall" {
  ...
  iso_file = "local:iso/debian-${var.debian_version}-amd64-netinst.iso"
  http_directory = "."
  boot_wait      = "10s"
  ...
  vm_id = var.vm_id
  vm_name  = "debian-${var.debian_version}-amd64-template"
  cpu_type = "host"
  os       = "l26"
  memory   = 2048
  cores    = 2
...
build {
  name = "debian-${var.debian_version}-amd64-template"
